 ...
 * @see gov.customs.jm.data.PreBhlList
...
public class PreBhlListHome {
	...
	public PreBhlList findById(gov.customs.jm.data.PreBhlListId id) {
		log.debug("getting PreBhlList instance with id: " + id);
		try {
			PreBhlList instance = (PreBhlList) sessionFactory
					.getCurrentSession().get("gov.customs.jm.data.PreBhlList",
							id);
			if (instance == null) {
	...
	public List<PreBhlList> findByExample(PreBhlList instance) {
		...
		try {
			List<PreBhlList> results = (List<PreBhlList>) sessionFactory
					.getCurrentSession()
					.createCriteria("gov.customs.jm.data.PreBhlList")
